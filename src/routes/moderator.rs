...
pub async fn mod_edit(token: CsrfToken, Query(params): Query<ModPageParams>, Form(form): Form<ModEditForm>, ) -> impl IntoResponse {
    ...
    let result = match form.action.as_str() {
        ACTIVATE_USER_ACTION => {
            db::toggle_user_active(form.id, true).await
        },
        DEACTIVATE_USER_ACTION => {
            db::toggle_user_active(form.id, false).await
        },
        PUBLISH_ADVERT_ACTION => {
            db::toggle_advert_publish(form.id, true).await
        },
        UNPUBLISH_ADVERT_ACTION => {
            db::toggle_advert_publish(form.id, false).await
        },
        _ => Err(())
    };
    if result.is_ok() {
        Redirect::to(&format!("/mod?advert_page={}&user_page={}", params.advert_page.unwrap_or(1), params.user_page.unwrap_or(1))).into_response()
    } else {
